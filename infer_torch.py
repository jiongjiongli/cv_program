...
class TorchInferManager:
    ...
    def add_letterbox(self, new_shape, img):
        ...
        self.print_duration('before resize')

        if shape[::-1] != new_unpad:  # resize
            # img = cv2.resize(img, new_unpad, interpolation=cv2.INTER_LINEAR)
            img = F.resize(img, size=new_unpad[::-1], interpolation=F.InterpolationMode.BILINEAR)
            # print(img.shape)

        self.print_duration('after resize')

        top, bottom = int(round(dh - 0.1)), int(round(dh + 0.1))
        left, right = int(round(dw - 0.1)), int(round(dw + 0.1))
        # img = cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT,
        #                          value=(114, 114, 114))  # add border
        img = F.pad(img, padding=(left, top, right, bottom), fill=114)
        # print(img.shape)
        # self.print_duration('pad')
    ...
    def infer(self, img):
        ...
        self.print_duration('after from_numpy')
        im = im.permute(2, 0, 1)

        # self.print_duration('before add_letterbox')
        ...
        # print(im.shape)
        # assert im.shape == torch.Size([1, 3, *self.imgsz])
...
def main():
    model = YOLO('/project/train/src_repo/ultralytics/model_backup/best.pt')
    # model = YOLO('/project/train/src_repo/ultralytics/model_backup/best.engine')

    for k, m in model.model.named_modules():
        if isinstance(m, (Detect)):
            m.forward = forward_nms
        elif isinstance(m, C2f):
            m.forward = m.forward_no_split
    ...
    # manager = TorchInferManager(model.predictor.model, model.predictor.model.imgsz)

    manager = TorchInferManager()
    durations = []
