...
class YOLOWriter:
    ...
    '''
    def bnd_box_to_yolo_line(self, box, class_list, img_size):
        x_min = box['xmin']
        x_max = box['xmax']
        y_min = box['ymin']
        y_max = box['ymax']
    ...
    def save(self, box_list, class_list, img_size, target_file):
        target_file = Path(target_file).resolve()
        assert target_file.suffix == TXT_EXT, 'Unsupported file format'

        with open(target_file.as_posix(), 'w', encoding=ENCODE_METHOD) as out_file:
            for box in box_list:
                class_index, x_center, y_center, w, h = self.bnd_box_to_yolo_line(box, class_list, img_size)
